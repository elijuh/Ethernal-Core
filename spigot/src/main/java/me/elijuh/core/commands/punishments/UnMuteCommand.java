        ...
        if (args.length > 1) {
            if (databaseManager.isPunished(args[0], Punishment.MUTE)) {
                ...
                databaseManager.remove(databaseManager.getUUID(args[0]), new PunishmentInfo(
                        Punishment.MUTE, true, -1, reason.toString(), p.getName(), args[0], display, databaseManager.getDisplay(args[0])));
            } else {
    ...
    public void onConsole(CommandSender sender, String[] args) {
        if (args.length > 1) {
            if (databaseManager.isPunished(args[0], Punishment.MUTE)) {
                ...
                databaseManager.remove(databaseManager.getUUID(args[0]), new PunishmentInfo(
                        Punishment.MUTE, true, -1, reason.toString(), "Console", args[0], display, databaseManager.getDisplay(args[0])));
            } else {
